...
class SolanaSwaps:
    ...
    async def fetch_wallet_balance_sol(self):
        ...
        async with self.session.post(self.api_key, json=payload, headers=headers) as resp:
            if resp.status == 200:
    ...
    def _normalize_account_key(self, account_key: Any) -> str:
        if isinstance(account_key, dict):
            return str(
                account_key.get("pubkey")
                or account_key.get("pubKey")
                or account_key.get("key")
                or account_key.get("address")
                or ""
            )
        return str(account_key)

    def _extract_wallet_balance_details(self, result: dict) -> Optional[dict]:
        meta = result.get("meta", {})
        message = result.get("transaction", {}).get("message", {})
        account_keys = [
            self._normalize_account_key(account_key)
            for account_key in message.get("accountKeys", [])
        ]
        if self.wallet_address not in account_keys:
            logging.error(
                "Wallet %s is missing from confirmed transaction account keys.",
                self.wallet_address,
            )
            return None
        ...
        if wallet_index >= len(pre_balances) or wallet_index >= len(post_balances):
            logging.error(
                "Wallet balance arrays are missing index %s for %s.",
                wallet_index,
                self.wallet_address,
            )
            return None

        pre_wallet_balance = int(pre_balances[wallet_index])
        post_wallet_balance = int(post_balances[wallet_index])
        return {
            "pre_wallet_balance": pre_wallet_balance,
            "wallet_balance": post_wallet_balance,
            "wallet_delta_lamports": post_wallet_balance - pre_wallet_balance,
            "fee_lamports": int(meta.get("fee") or 0),
        }

    def _instruction_error_result(self, err: Any) -> str:
        instruction_error = err.get("InstructionError") if isinstance(err, dict) else None
        custom_code = None
        if isinstance(instruction_error, list) and len(instruction_error) >= 2:
            details = instruction_error[1]
            if isinstance(details, dict):
                custom_code = details.get("Custom")
        if custom_code is None:
            return "InstructionError"
        return f"InstructionError:{int(custom_code)}"
    ...
    async def get_swap_tx(self, tx_id: str, mint_token: str, tx_type: str = "buy", max_retries: int = 4, retry_interval: float = 0.2) -> Optional[str]:
        ...
        while attempt < max_retries:
            try:
                ...
                async with self.session.post(self.api_key, json=payload, headers=headers, timeout=10) as response:
                    ...
                    if data and data.get('result') is not None:
                        ...
                        meta = result.get("meta", {})
                        err = meta.get("err")
                        if isinstance(err, dict) and err.get("InstructionError"):
                            logging.info(f"{cc.RED}Instruction error occurred for {tx_id}: {err}")
                            return self._instruction_error_result(err)
                        
                        post_token_balances = meta.get("postTokenBalances", [])
                        wallet_balance_details = self._extract_wallet_balance_details(result)
                        if wallet_balance_details is None:
                            raise ValueError("confirmed transaction is missing wallet balance details")
                                ...
                                if post_token_balance.get("mint") == mint_token:
                                    if post_token_balance.get('owner') == self.wallet_address:
                                        ...
                                        return {
                                            ...
                                            "price": price,
                                            "sol_balance": wallet_balance_details["wallet_balance"],
                                            "wallet_balance": wallet_balance_details["wallet_balance"],
                                            "wallet_delta_lamports": wallet_balance_details["wallet_delta_lamports"],
                                            "pre_wallet_balance": wallet_balance_details["pre_wallet_balance"],
                                            "cost_lamports": max(0, -wallet_balance_details["wallet_delta_lamports"]),
                                            "fee_lamports": wallet_balance_details["fee_lamports"],
                                        }
                        elif tx_type == "sell":
                            price = self.process_log(data)
                            return {
                                "balance": wallet_balance_details["wallet_balance"],
                                "wallet_balance": wallet_balance_details["wallet_balance"],
                                "wallet_delta_lamports": wallet_balance_details["wallet_delta_lamports"],
                                "pre_wallet_balance": wallet_balance_details["pre_wallet_balance"],
                                "price": price,
            ...
            if attempt < max_retries:
                ...
                return {"balance": 0, "price": 0, "wallet_balance": None, "wallet_delta_lamports": None}
        return None
